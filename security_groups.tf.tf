...
resource "aws_security_group" "ad_connector" {
  ...
  # NOTE: Requires NAT Gateway in VPC2 to route 0.0.0.0/0 traffic from private subnets to internet
  # TODO: create NAT Gateway in network_infra.tf if not already present
  egress {
...
resource "aws_security_group" "workspaces" {
  ...
  egress {
    ...
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] #TODO: restrict this to WorkSpaces IP ranges
  }
  tags = {
    Name        = "WorkSpaces-SG"
    Environment = "Production"
